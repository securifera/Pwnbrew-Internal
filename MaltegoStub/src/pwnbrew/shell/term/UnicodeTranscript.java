...
class UnicodeTranscript {
    ...
    public int externalToInternalRow(int extRow) {
        if (extRow < -mActiveTranscriptRows || extRow > mScreenRows) {
            ...
            Logger.getLogger(UnicodeTranscript.class.getName()).log(Level.WARNING, errorMessage);
    ...
    public void blockSet(int sx, int sy, int w, int h, int val, int style) {
        if (sx < 0 || sx + w > mColumns || sy < 0 || sy + h > mScreenRows) {
            Logger.getLogger(UnicodeTranscript.class.getName()).log(Level.WARNING, "illegal arguments! " + sx + " " + sy + " " + w + " " + h + " " + val + " " + mColumns + " " + mScreenRows);
            throw new IllegalArgumentException();
        }
    ...
    public char[] allocateBasicLine(int row, int columns) {
    ...
    public boolean setChar(int column, int row, int codePoint) {
        if (row >= mScreenRows || column >= mColumns) {
            Logger.getLogger(UnicodeTranscript.class.getName()).log(Level.WARNING, "illegal arguments! " + row + " " + column + " " + mScreenRows + " " + mColumns);
            throw new IllegalArgumentException();
        }
        row = externalToInternalRow(row);
